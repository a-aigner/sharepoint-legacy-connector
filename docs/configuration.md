...
| `SP_NTLM_PRIME_CONNECTION` | bool | `True` | Retry a 401'd SOAP POST once on a connection a contentless request has authenticated. Two priming attempts at most — an empty POST, then a bodyless GET — and a refusal on the GET ends it, so it never spends a third attempt on a genuinely bad credential. See [troubleshooting](troubleshooting.md). |
| `SP_NTLM_PRIME_URL` | str | *(empty)* | A URL that **demands** a credential, used to get the NTLM handshake done before the SOAP POST. Only needed where anonymous access is enabled: there every contentless request to `_vti_bin` is served without a challenge, so no handshake ever starts, while the one request the farm *does* challenge is the SOAP POST whose handshake cannot complete — priming has nowhere to stand. Blank tries `/_layouts/viewlsts.aspx` then `/_layouts/settings.aspx`. Anything that 401s and then serves this account will do. |
| `SP_NTLM_SEND_CBT` | bool | `True` | Bind the NTLM Type 3 to the server's TLS certificate (a Channel Binding Token). Leave on: IIS with Extended Protection set to *Required* refuses a Type 3 without one. Set **false** when TLS is terminated by a load balancer or reverse proxy rather than the farm — the client binds to the certificate *it* sees, which is then not the one the server expects, and the Type 3 is refused with a bare re-challenge that is indistinguishable from a wrong password. https only. |
